...
export default function EditTransactionModal({ isOpen, onClose, onSubmit, transaction, wallets }) {
  const { user } = useAuth();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto bg-white dark:bg-gray-800 dark:border-gray-700">
        <DialogHeader>
          <DialogTitle className="dark:text-gray-100">تعديل المعاملة</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          ...
          {/* Type */}
          <div>
            <Label htmlFor="type" className="dark:text-gray-300">نوع المعاملة</Label>
            <Select 
              value={formData.type} 
          ...
          <div>
            <Label htmlFor="wallet" className="dark:text-gray-300">
              {formData.type === 'transfer' ? 'من المحفظة' : 'المحفظة'}
            </Label>
            <Select 
